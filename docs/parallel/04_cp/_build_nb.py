...
3. **zigzag 负载均衡**（[`README` 第 3 节](./README.md)）：演示朴素切分的负载不均，以及 Megatron `get_pretrain_batch_on_this_cp_rank` 的 `2*cp` 块对称分配。
...
md(r"""## Part 2 · zigzag 负载均衡（[`README` 第 3 节](./README.md)）
...
两者都把 `O(s²)` 单卡 attention 摊到 `cp` 张卡，且每卡只存 `1/cp` 的 Q/K/V。对应正文 [`README` 第 2 节](./README.md) 的对比表。
