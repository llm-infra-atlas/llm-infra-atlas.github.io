...
for N in (8192, 16384, 32768, 65536):
    tok = (N - 32) // 16 + 1 + 16 * 64 + 512
    print(f"N={N:6d}: cmp {(N-32)//16+1:5d} + sel {16*64} + win 512 = {tok:5d}  ({N/tok:.1f}x)")
# N=  8192: cmp   511 + sel 1024 + win 512 =  2047  (4.0x)
# N= 16384: cmp  1023 + sel 1024 + win 512 =  2559  (6.4x)
# N= 32768: cmp  2047 + sel 1024 + win 512 =  3583  (9.1x)
# N= 65536: cmp  4095 + sel 1024 + win 512 =  5631  (11.6x)   ← 精确复现论文 Table 4
```

这个算术揭示了一件论文没有强调、但很关键的事实：**在长上下文下，NSA 的 decode 流量实际是由压缩分支（4095）主导的，而不是选择分支（1024）**。压缩分支是唯一一个仍会随 $N$ 线性增长的部分。**DeepSeek-V4 的 CSA 正是针对这一点做了改进**，做法是先压缩、再在压缩后的序列上做选择（[`05`](./05_sparse_dsa_frontier.md) §5）。
